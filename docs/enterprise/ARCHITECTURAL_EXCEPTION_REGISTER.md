...
| Métrica | Quantidade |
|---------|------------|
| **Total de ressalvas registradas** | **89** |
| Prioridade **Alta** | **14** |
| Prioridade **Média** | **21** |
| Prioridade **Baixa** | **54** |
| Criticidade **Bloqueante** (histórico GATE-ARCH-02 + TISS-XML-GATE-01) | **5** |
| Criticidade **Não bloqueante** | **84** |
| Status **Resolvida** | **11** |
| Status **Planejada** | **3** |
| Status **Aceita** | **74** |
| Status **Aberta** | **1** |
| Pendentes (Aberta + Aceita + Planejada) | **78** |
...
| AER-GA03-A1 | Dual-path Captura (estrutural vs funcional) | GATE-ARCH-03 | Architecture | Alta | Não bloqueante | **Resolvida** | EPC-24E (cutover) |
...
| AER-SR-B1 | Escape hatch `getSchedulerRuntimePort()` | INF-07 | Architecture | Baixa | Não bloqueante | Aceita | API interna / proibir produto |
...
#### Alta
- **AER-GA03-A1** Dual-path Captura — **Resolvida** em **EPC-24E** (2026-08-11): pipeline único via `getEnterpriseRuntime()`; flags `*Fallback` removidas; engines legado permanecem apenas como implementação interna dos gateways autorizados; Server Fns sem execução direta do legado; Dual Path eliminado.

##### AER-GA03-A1 — Resolvida
- **Título:** Dual-path Captura (estrutural vs funcional)
- **Descrição:** Coexistência de pipeline operacional Capture (engines locais) e caminho estrutural Enterprise Runtime, com flags de fallback Dual Path durante a convergência ARC-24 / EPC-24A–D.
- **Origem:** GATE-ARCH-03
- **Categoria:** Architecture
- **Prioridade:** Alta
- **Criticidade:** Não bloqueante
- **Status:** **Resolvida** em **EPC-24E** (Enterprise Runtime Final Cutover)
- **Evidência:** `src/lib/capture/enterprise/*-via-enterprise.ts` (sem `*Fallback`); `capture-runtime-binding.ts` (`singlePipeline: true`); Server Fns (`capture-server` / `review-server` / `tiss-server`) apenas via gateways; `docs/enterprise/EPC24E_ENTERPRISE_RUNTIME_FINAL_CUTOVER.md`; teste `epc-24e-enterprise-runtime-final-cutover.test.ts`
...
### Atualização TISS-09 (03/08/2026)

1. **TISS-09 implementado** — Enterprise XSD Runtime (`src/lib/enterprise/xsd-runtime/`).  
2. Cadeia oficial: Produto → Enterprise Runtime → TISS Runtime → `TISSCatalogPort` → `RulePackEnginePort` → `XMLRuntimePort` → `XMLGenerationRuntimePort` → `XMLSerializerRuntimePort` → `XMLSchemaRuntimePort` → `XMLValidationRuntimePort` → `XSDRuntimePort` → Adapter → Store → Canonical XSD Runtime Result.  
3. Resposta estrutural apenas — **`officialXsdLoaded = false`**, **`realXsdLoaded = false`**, **`realValidationAvailable = false`**, **`officialNamespacesLoaded = false`**, **`officialSchemasLoaded = false`**, **`schemaParsingEnabled = false`**, **`schemaValidationEnabled = false`**, **`runtimeReady = true`** hardcoded; sem XSD oficial / validação real / XML TISS/ANS / namespace / SOAP.  
4. Integração à cadeia XML via composition root (Enterprise/TISS Runtime); módulos XML Runtime / Generation / Serializer / Schema / Validation não importam XSD Runtime.  
5. **Novas ressalvas Baixa:** **AER-XSD-B1…B2** (Aceitas, não bloqueantes).  
6. **AER-XMLVAL-B1…B2** / **AER-XMLSCH-B1…B2** / **AER-XMLSER-B1…B2** / **AER-XMLGEN-B1…B2** / **AER-XMLRT-B1…B3** permanecem Aceitas (sem regressão).  
7. Indicador permanente novo: **XSD Runtime Coverage = 100%**.  
8. Sprint **TISS-09 encerrada oficialmente**. Próxima Sprint obrigatória: **TISS-XSD-GATE-01**.  
9. Documentos: `TISS-09_ENTERPRISE_XSD_RUNTIME.md`, `TISS-09_XSD_RUNTIME_ARCHITECTURE.md`, `TISS-09_XSD_RUNTIME_CERTIFICATION.md`.

#### AER-XSD-B1
- **Título:** Escape hatch `getXSDRuntimePort()`  
- **Descrição:** Runtime expõe o Port diretamente (paralelo a `getXMLValidationRuntimePort` / AER-XMLVAL-B1). Cadeia oficial permanece Runtime → TISS Runtime → XMLRuntimePort → XMLGenerationRuntimePort → XMLSerializerRuntimePort → XMLSchemaRuntimePort → XMLValidationRuntimePort → XSDRuntimePort.  
- **Origem:** TISS-09 · **Prioridade:** Baixa · **Status:** Aceita · **Sprint:** API interna / proibir produto

#### AER-XSD-B2
- **Título:** Barrel exporta Store + `getStore()` no Adapter (XSD)  
- **Descrição:** `xsd-runtime/index.ts` reexporta `InMemoryXSDRuntimeStore`; adapters expõem `getStore()` fora do Port — superfície de uso indevido (sem consumidor produto atual). Espelho de **AER-XMLVAL-B2**.  
- **Origem:** TISS-09 · **Prioridade:** Baixa · **Status:** Aceita · **Sprint:** Restringir superfície pública

### Atualização TISS-XSD-GATE-01 (03/08/2026)

1. **Auditoria exclusivamente de leitura** — nenhuma alteração de código/Runtime/Provider/Adapter/Store/Models/Factory/Registry/testes/comportamento.  
2. **Cadeia oficial reconfirmada íntegra:** Produto → Enterprise Runtime → TISS Runtime → `TISSCatalogPort` → `RulePackEnginePort` → `XMLRuntimePort` → `XMLGenerationRuntimePort` → `XMLSerializerRuntimePort` → `XMLSchemaRuntimePort` → `XMLValidationRuntimePort` → `XSDRuntimePort` → Adapter → Store → Canonical XSD Runtime Result.  
3. **Sem Provider paralelo / sem Adapter paralelo / sem Runtime paralelo / sem bypass Enterprise** ao `XSDRuntimePort`.  
4. **XSD Runtime permanece estrutural e desacoplado:** `officialXsdLoaded = false` / `realXsdLoaded = false` / `realValidationAvailable = false` / `officialNamespacesLoaded = false` / `officialSchemasLoaded = false` / `schemaParsingEnabled = false` / `schemaValidationEnabled = false` / `runtimeReady = true`; sem XSD oficial; sem leitura/parser de XSD; sem namespace oficial; sem validação XSD; sem XML TISS/ANS; sem SOAP; sem Reader/Parser/Validator; sem operadora/contrato/tenant; sem regras ANS.  
5. **Ressalvas resolvidas nesta Gate:** nenhuma.  
6. **Ressalvas Baixa confirmadas Aceitas (não bloqueantes):** **AER-XSD-B1…B2**, **AER-XMLVAL-B1…B2**, **AER-XMLSCH-B1…B2**, **AER-XMLSER-B1…B2**, **AER-XMLGEN-B1…B2**, **AER-XMLRT-B1…B3**.  
7. **Nenhuma nova Architectural Exception encontrada.** / **nenhuma regressão** / **ECS-01 íntegro**.  
8. **Gates:** Build PASS · TypeScript PASS · ESLint PASS · Smoke PASS · Enterprise PASS (66/66) · Capture PASS (198/1 skipped) · `enterprise:xsd-runtime:test` PASS (18/18).  
9. **Indicadores XML (sem regressão):** Coverage **50%** · Runtime Coverage **100%** · Generation Runtime Coverage **100%** · Serializer Runtime Coverage **100%** · Schema Runtime Coverage **100%** · Validation Runtime Coverage **100%** · XSD Runtime Coverage **100%** · Legacy Components **1** · Enterprise Compliance **95%**.  
10. **Parecer:** **GO COM RESSALVAS**.  
11. **Encerramento:** Sprint **TISS-09** declarada encerrada oficialmente; Enterprise XSD Runtime oficialmente certificado.  
...
### Atualização INF-07 — Enterprise Scheduler Runtime (03/08/2026)

1. **Natureza:** foundation estrutural ECS-01 — sem Scheduler real / Cron / Timer / Workers / processamento paralelo.  
2. **Scheduler Runtime oficial:** `SchedulerRuntimePort` via `createSchedulerRuntimePort()`; Factory + Registry únicos; Adapters Default/Enterprise/Mock.  
3. **Enterprise Runtime:** `getSchedulerRuntimePort()` + health `schedulerRuntimeOk`.  
4. **Queue / Worker Runtime:** dependência `getSchedulerRuntimePort()` preparada; **sem** schedule/cancel/execução. Scheduler recebe `getQueueRuntimePort()` + `getWorkerRuntimePort()` preparados; **sem** consumo/orquestração.  
5. **TISS Runtime:** dependência obrigatória `getSchedulerRuntimePort()` preparada; **sem** utilização funcional.  
6. **Novas ressalvas Baixa:** **AER-SR-B1…B2** (Aceitas, não bloqueantes).  
7. **Cobertura:** `enterprise:scheduler-runtime:test`.  
8. **Sprint encerrada** com parecer **GO COM RESSALVAS**.  
9. Documentos: `INF-07_ENTERPRISE_SCHEDULER_RUNTIME.md`, `INF-07_SCHEDULER_RUNTIME_ARCHITECTURE.md`, `INF-07_SCHEDULER_RUNTIME_CERTIFICATION.md`.  
10. **Roadmap:** INF-07A **não iniciada**.

#### AER-SR-B1
- **Título:** Escape hatch `getSchedulerRuntimePort()`  
- **Descrição:** Runtime expõe o Port diretamente (paralelo a `getWorkerRuntimePort` / AER-WR-B1). Cadeia oficial permanece Produto → Enterprise Runtime → SchedulerRuntimePort. Queue/Worker/TISS recebem apenas dependência preparada.  
- **Origem:** INF-07 · **Prioridade:** Baixa · **Status:** Aceita · **Sprint:** API interna / proibir produto

#### AER-SR-B2
- **Título:** Barrel exporta Store + `getStore()` no Adapter (Scheduler Runtime)  
- **Descrição:** `scheduler-runtime/index.ts` reexporta `InMemorySchedulerRuntimeStore`; adapters expõem `getStore()` fora do Port — superfície de uso indevido (sem consumidor produto atual). Espelho de **AER-WR-B2**.  
- **Origem:** INF-07 · **Prioridade:** Baixa · **Status:** Aceita · **Sprint:** Restringir superfície pública

### Atualização INF-08 — Enterprise Persistent Queue Runtime (03/08/2026)

1. **Natureza:** foundation estrutural ECS-01 — sem RabbitMQ / Kafka / Azure Service Bus / Azure Queue / Redis Streams / BullMQ / persistência real.  
2. **Persistent Queue Runtime oficial:** `PersistentQueueRuntimePort` via `createPersistentQueueRuntimePort()`; Factory + Registry únicos; Adapters Default/Enterprise/Mock.  
3. **Enterprise Runtime:** `getPersistentQueueRuntimePort()` + health `persistentQueueRuntimeOk`.  
4. **Queue / Worker / Scheduler Runtime:** dependência `getPersistentQueueRuntimePort()` preparada; **sem** persist/release/execução. Persistent Queue recebe `getQueueRuntimePort()` + `getWorkerRuntimePort()` + `getSchedulerRuntimePort()` preparados; **sem** consumo.  
5. **TISS Runtime:** dependência obrigatória `getPersistentQueueRuntimePort()` preparada; **sem** utilização funcional.  
6. **Novas ressalvas Baixa:** **AER-PQR-B1…B2** (Aceitas, não bloqueantes).  
7. **Cobertura:** `enterprise:persistent-queue-runtime:test`.  
8. **Sprint encerrada** com parecer **GO COM RESSALVAS**.  
9. Documentos: `INF-08_ENTERPRISE_PERSISTENT_QUEUE_RUNTIME.md`, `INF-08_PERSISTENT_QUEUE_RUNTIME_ARCHITECTURE.md`, `INF-08_PERSISTENT_QUEUE_RUNTIME_CERTIFICATION.md`.  
10. **Roadmap:** INF-08A **não iniciada**.

#### AER-PQR-B1
- **Título:** Escape hatch `getPersistentQueueRuntimePort()`  
- **Descrição:** Runtime expõe o Port diretamente (paralelo a `getSchedulerRuntimePort` / AER-SR-B1). Cadeia oficial permanece Produto → Enterprise Runtime → PersistentQueueRuntimePort. Queue/Worker/Scheduler/TISS recebem apenas dependência preparada.  
- **Origem:** INF-08 · **Prioridade:** Baixa · **Status:** Aceita · **Sprint:** API interna / proibir produto

#### AER-PQR-B2
- **Título:** Barrel exporta Store + `getStore()` no Adapter (Persistent Queue Runtime)
- **Descrição:** `persistent-queue-runtime/index.ts` reexporta `InMemoryPersistentQueueRuntimeStore`; adapters expõem `getStore()` fora do Port — superfície de uso indevido (sem consumidor produto atual). Espelho de **AER-SR-B2**.
- **Origem:** INF-08 · **Prioridade:** Baixa · **Status:** Aceita · **Sprint:** Restringir superfície pública

### Atualização INF-08A — Enterprise Persistent Queue Gate (03/08/2026)

1. **Natureza:** auditoria/certificação exclusivamente — **zero** alteração de Foundation, Runtime, Provider, Adapter, Store, Factory, Registry, Enterprise Runtime, Queue/Worker/Scheduler/TISS Runtime, Capture ou comportamento.
2. **Cadeia oficial reconfirmada:** Produto → Enterprise Runtime → `PersistentQueueRuntimePort` → Adapter → `InMemoryPersistentQueueRuntimeStore` → Canonical Persistent Queue Result.
3. **Sem** Runtime/Provider/Adapter/Factory/Registry paralelo de Persistent Queue; **sem** bypass na cadeia Enterprise.
4. **Sem** RabbitMQ / Kafka / Azure Service Bus / Azure Queue / Redis Streams / BullMQ / DLQ / Retry / Delay / Priority Queue / persistência real / Workers reais / Scheduler real.
5. **Escape hatches reconfirmados:** **AER-PQR-B1** (`getPersistentQueueRuntimePort`) · **AER-PQR-B2** (barrel/`getStore()`) — continuam **Aceitas**.
6. **Nova ressalva Média:** **AER-PQR-T1** — 10 suítes Enterprise pré-INF-08 que chamam `createTISSRuntimePort` sem `getPersistentQueueRuntimePort` falham (1 teste de fluxo cada); cadeia via Enterprise Runtime permanece PASS; suíte oficial PQR **21/21 PASS**.
7. **Gates:** build / `tsc --noEmit` / lint (0 errors) / smoke **PASS**; Capture **198 pass / 1 skipped** **PASS**; Enterprise **61/71** (falhas = **AER-PQR-T1**).
8. **Parecer:** **GO COM RESSALVAS**. INF-08 encerrada e certificada. INF-09 **não iniciada**.
9. Documento: `INF-08A_ENTERPRISE_PERSISTENT_QUEUE_GATE_CERTIFICATION.md`.
...
### Atualização INF-09 — Enterprise Observability Runtime (03/08/2026)

1. **Natureza:** foundation estrutural ECS-01 — sem OpenTelemetry / Application Insights / Azure Monitor / Prometheus / Grafana / Elastic / Datadog / New Relic / Loki / Jaeger / logs / métricas / tracing / alertas / dashboards reais.  
2. **Observability Runtime oficial:** `ObservabilityRuntimePort` via `createObservabilityRuntimePort()`; Factory + Registry únicos; Adapters Default/Enterprise/Mock.  
3. **Enterprise Runtime:** `getObservabilityRuntimePort()` + health `observabilityRuntimeOk`.  
4. **Queue / Worker / Scheduler / Persistent Queue Runtime:** dependência `getObservabilityRuntimePort()` preparada; **sem** observe/release/execução. Observability recebe `getQueueRuntimePort()` + `getWorkerRuntimePort()` + `getSchedulerRuntimePort()` + `getPersistentQueueRuntimePort()` + `getTISSRuntimePort()` preparados; **sem** consumo.  
5. **TISS Runtime:** dependência obrigatória `getObservabilityRuntimePort()` preparada; **sem** utilização funcional.  
6. **Novas ressalvas Baixa:** **AER-OBS-B1…B2** (Aceitas, não bloqueantes).  
7. **Harnesses:** atualizados proativamente com `getObservabilityRuntimePort` — **sem AER-OBS-T1**.  
8. **Cobertura:** `enterprise:observability-runtime:test`.  
9. **Sprint encerrada** com parecer **GO COM RESSALVAS** (gates a preencher).  
10. Documentos: `INF-09_ENTERPRISE_OBSERVABILITY_RUNTIME.md`, `INF-09_OBSERVABILITY_RUNTIME_ARCHITECTURE.md`, `INF-09_OBSERVABILITY_RUNTIME_CERTIFICATION.md`.  
11. **Roadmap:** INF-09A **não iniciada**.

#### AER-OBS-B1
- **Título:** Escape hatch `getObservabilityRuntimePort()`  
- **Descrição:** Runtime expõe o Port diretamente (paralelo a `getPersistentQueueRuntimePort` / AER-PQR-B1). Cadeia oficial permanece Produto → Enterprise Runtime → ObservabilityRuntimePort. Queue/Worker/Scheduler/Persistent Queue/TISS recebem apenas dependência preparada.  
- **Origem:** INF-09 · **Prioridade:** Baixa · **Status:** Aceita · **Sprint:** API interna / proibir produto

#### AER-OBS-B2
- **Título:** Barrel exporta Store + `getStore()` no Adapter (Observability Runtime)
