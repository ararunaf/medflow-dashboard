...
describe("EPC-21C EnterpriseTissMappingRegistryEngine", () => {
  ...
  it("constroi engine com as sete engines Enterprise autorizadas", () => {
    assert.ok(registry.canonicalEngine);
    assert.ok(registry.discoveryEngine);
    assert.ok(registry.genericVocabularyEngine);
    assert.ok(registry.genericTissEngine);
    assert.ok(registry.genericTissIntegrationEngine);
    assert.ok(registry.workflowEngine);
    assert.ok(registry.masterOrchestrationEngine);
  });

  it("reutiliza engines anteriores por identidade referencial", () => {
    assert.strictEqual(registry.canonicalEngine, canonical);
    assert.strictEqual(registry.discoveryEngine, discovery);
    assert.strictEqual(registry.genericVocabularyEngine, genericVocabulary);
    assert.strictEqual(registry.genericTissEngine, tiss);
    assert.strictEqual(registry.genericTissIntegrationEngine, tissIntegration);
    assert.strictEqual(registry.workflowEngine, workflow);
    assert.strictEqual(registry.masterOrchestrationEngine, masterOrchestration);
  });
  ...
  it("getCapabilities ativa discovery, canonical e registry, mantem demais false", () => {
  ...
  it("nao contem storage, cache, consulta, parser, algoritmo, IA ou runtime", () => {
    const keys = Object.keys(registry);
    for (const k of ["store", "cache", "index", "query", "parser", "runtime"]) {
