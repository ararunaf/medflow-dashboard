...
describe("EPC-21E EnterpriseGenericTissMappingEngine", () => {
  ...
  const engine = new EnterpriseGenericTissMappingEngine(
    query,
    registry,
    canonical,
    discovery,
  ...
  it("constroi engine com as nove engines Enterprise autorizadas", () => {
    assert.ok(engine.queryEngine);
    assert.ok(engine.registryEngine);
    assert.ok(engine.canonicalEngine);
  ...
  it("reutiliza engines anteriores por identidade referencial", () => {
    assert.strictEqual(engine.queryEngine, query);
    assert.strictEqual(engine.registryEngine, registry);
    assert.strictEqual(engine.canonicalEngine, canonical);
  ...
  it("contem apenas nove propriedades readonly, construtor e getCapabilities", () => {
    ...
    assert.ok(keys.includes("queryEngine"));
  ...
  it("getCapabilities ativa todas as mapping capabilities", () => {
    ...
    assert.strictEqual(caps.tissGenericMappingEngineImplemented, true);
  });
  ...
  it("expoem query, registry, canonical e discovery como gateway oficial", () => {
    assert.strictEqual(engine.queryEngine.getCapabilities().tissMappingQueryEngineImplemented, true);
    assert.strictEqual(engine.registryEngine.getCapabilities().tissMappingRegistryImplemented, true);
    assert.strictEqual(engine.canonicalEngine.getCapabilities().tissMappingCanonicalModelImplemented, true);
    assert.strictEqual(engine.discoveryEngine.getCapabilities().tissMappingDiscoveryImplemented, true);
  });
});
