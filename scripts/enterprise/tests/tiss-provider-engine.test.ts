 ...
 *         + aus�ncia de bypass / l�gica espec�fica de operadora/contrato/tenant
 */
...
describe("TISS-01 TISSProviderPort contract", () => {
  ...
  it("DefaultTISSProviderAdapter � o adapter enterprise oficial", () => {
  ...
  it("resolve-profile e resolve-provider retornam refer�ncias can�nicas opacas", async () => {
    const port = createTISSProviderPort({ provider: "enterprise" });
  ...
  it("timeout � implementado e retorna TISS_TIMEOUT", async () => {
...
describe("TISS-01 cadeia Enterprise / TISS Runtime / Provider", () => {
  it("Enterprise Runtime exp�e TISS Provider + TISS Runtime", async () => {
    resetEnterpriseRuntimeForTests();
  ...
  it("Enterprise Runtime permanece ponto �nico; Capture desacoplado de TISS real", async () => {
    resetEnterpriseRuntimeForTests();
...
describe("TISS-01 auditoria � sem bypass / sem l�gica de operadora", () => {
  it("m�dulo tiss-provider n�o cont�m backends / operadoras / XML dispatch", () => {
  ...
  it("m�dulo tiss-runtime usa exclusivamente TISSProviderPort", () => {
  ...
  it("n�o existe if/switch por operadora/tenant/cliente/contrato no tiss-provider", () => {
    const moduleDir = join(repoRoot, "src/lib/enterprise/tiss-provider");
