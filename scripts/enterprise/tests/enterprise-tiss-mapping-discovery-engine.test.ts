...
describe("EPC-21A EnterpriseTissMappingDiscoveryEngine", () => {
  ...
  const engine = new EnterpriseTissMappingDiscoveryEngine(
    genericVocabulary,
    tiss,
    tissIntegration,
    workflow,
  ...
  it("constroi engine com as cinco engines Enterprise", () => {
  ...
  it("reutiliza genericVocabularyEngine por identidade referencial", () => {
  ...
  it("contem apenas cinco propriedades readonly, construtor e getCapabilities", () => {
  ...
  it("getCapabilities ativa apenas tissMappingDiscoveryImplemented", () => {
    const caps = engine.getCapabilities();
    assert.strictEqual(caps.tissMappingDiscoveryImplemented, true);
    assert.strictEqual(caps.tissMappingCanonicalModelImplemented, false);
  ...
  it("nao importa engines internas da EPC-20", () => {
    // A engine consome exclusivamente o Gateway EnterpriseGenericTissVocabularyEngine.
    // O TypeScript garante a ausência de imports diretos das engines internas de Vocabulary.
    assert.strictEqual(engine.genericVocabularyEngine.getCapabilities().tissGenericVocabularyEngineImplemented, true);
    assert.ok(engine.genericVocabularyEngine.queryEngine);
    assert.ok(engine.genericVocabularyEngine.registryEngine);
    assert.ok(engine.genericVocabularyEngine.canonicalEngine);
    assert.ok(engine.genericVocabularyEngine.discoveryEngine);
  });
});
