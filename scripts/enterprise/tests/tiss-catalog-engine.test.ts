 ...
 *         + Modelos Can�nicos / Cat�logo
 *         + timeout / retry / cancelamento / erros
 *         + aus�ncia de bypass / l�gica espec�fica de operadora/vers�o
 */
...
describe("TISS-02 TISSCatalogPort contract", () => {
  ...
  it("DefaultTISSCatalogAdapter � o adapter enterprise oficial", () => {
  ...
  it("test / default / mock s�o resolvidos pelo factory", async () => {
  ...
  it("modelos can�nicos e cat�logo m�nimo est�o dispon�veis via Port", async () => {
  ...
  it("InMemoryTISSCatalog � o Catalog Store oficial", () => {
  ...
  it("timeout � implementado e retorna TISS_CATALOG_TIMEOUT", async () => {
  ...
  it("retry recupera falha transit�ria", async () => {
    const port = new DefaultTISSCatalogAdapter({
      provider: "enterprise",
      failAttempts: 1,
      defaultRetryCount: 2,
  ...
  it("Registry e Factory seguem ECS-01 (sem fallback silencioso)", () => {
    ...
    assert.throws(
      () =>
        factory.create({
          // @ts-expect-error � provider inv�lido
...
describe("TISS-02 cadeia Enterprise / TISS Runtime / Catalog", () => {
  it("Enterprise Runtime exp�e TISS Catalog + Runtime + Provider", async () => {
    resetEnterpriseRuntimeForTests();
...
describe("TISS-02 auditoria � sem bypass / sem l�gica de operadora/vers�o", () => {
  it("m�dulo tiss-catalog n�o cont�m backends / operadoras / XML dispatch", () => {
    ...
    for (const file of files) {
      ...
      for (const pattern of forbidden) {
        assert.equal(
          ...
          false,
          `Padr�o proibido ${pattern} em ${file}`,
        );
      }
    }
  });

  it("n�o existe if/switch por operadora/vers�o no tiss-catalog", () => {
    const moduleDir = join(repoRoot, "src/lib/enterprise/tiss-catalog");
    for (const file of collectTsFiles(moduleDir)) {
      ...
      assert.equal(
        /if\s*\(\s*vers[a�]o/i.test(codeWithoutComments),
        false,
        `if(versao) em ${file}`,
      );
      assert.equal(
        /switch\s*\(\s*vers[a�]o/i.test(codeWithoutComments),
        false,
  ...
  it("n�o existe cat�logo paralelo TISSCatalog fora de tiss-catalog", () => {
