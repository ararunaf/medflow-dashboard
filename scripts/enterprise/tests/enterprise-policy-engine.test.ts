...
describe("J-04 EnterprisePolicyEngine", () => {
  ...
  const engine = new EnterprisePolicyEngine(
    saga,
    orchestration,
    command,
    business,
  ...
  it("constrói engine com saga, orchestration, command e fachadas E, F, G, H e I", () => {
  ...
  it("reutiliza EnterpriseSagaEngine", () => {
    assert.strictEqual(engine.saga, saga);
    assert.ok(engine.saga.getCapabilities);
  });
  ...
  it("preserva a cadeia arquitetural: policy -> saga -> orchestration -> command -> fachadas", () => {
  ...
  it("não possui import direto de Adapters, Providers, Registries, Stores ou engines dos Blocos A-H", () => {
  ...
  it("não há ciclos de import: cada engine de ordem J consome somente engines de ordem inferior", () => {
    assert.strictEqual(engine.command, engine.saga.command);
    assert.strictEqual(engine.command, engine.orchestration.command);
    assert.strictEqual(engine.orchestration, engine.saga.orchestration);
    assert.ok(engine.saga.orchestration.command.getCapabilities);
  });

  it("getCapabilities retorna J-01, J-02, J-03 e J-04 ativas", () => {
