#!/usr/bin/env node
/**
 * S1-02 — Security Real Activation.
 *
 * Prova:
 *   RealTissSecurityRuntimeAdapter registrado como provider "real-tiss"
 *     → SecurityRuntimeFactory / SecurityRuntimeRegistry
 *     → createSecurityRuntimePort({ provider: "real-tiss" })
 ...
 * Sem segurança real, criptografia, assinatura digital, cadeia de custódia, Key Vault, HSM, SIEM, OpenTelemetry, LGPD, autenticação ou autorização.
 */
...
describe("S1-02 Security Real Activation — real-tiss sem alterar Enterprise Runtime", () => {
  it("factory + registry resolvem provider real-tiss", () => {
    const port = createSecurityRuntimePort({ provider: "real-tiss" });
  ...
  it("SecurityRuntimePort mantém shape canônico com 9 métodos", () => {
    const port = createSecurityRuntimePort({ provider: "real-tiss" });
    ...
    assert.equal(caps.securityEngineImplemented, false);
    assert.equal(caps.tissSecurityImplemented, false);
  ...
  it("factory e registry resolvem todos os 5 providers", () => {
    ...
    assert.equal(getSecurityRuntimeFactory().getRegistry().has("real-tiss"), true);
  });
  ...
  it("getEnterpriseRuntime continua singleton e inalterado", () => {
    ...
    assert.equal(r1, r2);
    // Enterprise Runtime não deve expor Security Runtime Port (não foi modificado).
    assert.equal(
      typeof (r1 as Record<string, unknown>).getSecurityRuntimePort,
      "undefined",
      "Enterprise Runtime não pode conter getSecurityRuntimePort",
    );
  ...
  it("RealTissSecurityRuntimeAdapter delega ao Default sem executar segurança real", async () => {
    const port = createSecurityRuntimePort({ provider: "real-tiss" });
    const opened = await port.openJob({});
    assert.equal(opened.ok, true);
    assert.equal(opened.job?.status, "job-open");
    assert.equal(opened.result?.securityEngineImplemented, false);
    assert.equal(opened.result?.tissSecurityImplemented, false);
