...
describe("EPC-21D EnterpriseTissMappingQueryEngine", () => {
  ...
  it("constroi engine com as oito engines Enterprise autorizadas", () => {
    assert.ok(query.registryEngine);
    assert.ok(query.canonicalEngine);
    assert.ok(query.discoveryEngine);
    assert.ok(query.genericVocabularyEngine);
    assert.ok(query.genericTissEngine);
    assert.ok(query.genericTissIntegrationEngine);
    assert.ok(query.workflowEngine);
    assert.ok(query.masterOrchestrationEngine);
  });

  it("reutiliza engines anteriores por identidade referencial", () => {
    assert.strictEqual(query.registryEngine, registry);
    assert.strictEqual(query.canonicalEngine, canonical);
    assert.strictEqual(query.discoveryEngine, discovery);
    assert.strictEqual(query.genericVocabularyEngine, genericVocabulary);
    assert.strictEqual(query.genericTissEngine, tiss);
    assert.strictEqual(query.genericTissIntegrationEngine, tissIntegration);
    assert.strictEqual(query.workflowEngine, workflow);
    assert.strictEqual(query.masterOrchestrationEngine, masterOrchestration);
  });
  ...
  it("getCapabilities ativa discovery, canonical, registry e query, mantem generic false", () => {
    ...
    assert.strictEqual(caps.tissMappingRegistryImplemented, true);
    assert.strictEqual(caps.tissMappingQueryEngineImplemented, true);
  ...
  it("nao implementa metodos de consulta, busca, execucao, cache ou persistencia", () => {
    const keys = Object.keys(query);
    for (const k of ["store", "cache", "index", "find", "search", "execute", "run"]) {
