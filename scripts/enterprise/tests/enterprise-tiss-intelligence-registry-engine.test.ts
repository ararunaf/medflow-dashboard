...
describe("EPC-22C EnterpriseTissIntelligenceRegistryEngine", () => {
  ...
  it("constroi engine com as oito engines Enterprise autorizadas", () => {
    assert.ok(intelligenceRegistry.canonicalEngine);
    assert.ok(intelligenceRegistry.discoveryEngine);
    assert.ok(intelligenceRegistry.genericMappingEngine);
    assert.ok(intelligenceRegistry.genericVocabularyEngine);
    assert.ok(intelligenceRegistry.genericTissEngine);
    assert.ok(intelligenceRegistry.genericTissIntegrationEngine);
    assert.ok(intelligenceRegistry.workflowEngine);
    assert.ok(intelligenceRegistry.masterOrchestrationEngine);
  });

  it("reutiliza engines anteriores por identidade referencial", () => {
    assert.strictEqual(intelligenceRegistry.canonicalEngine, intelligenceCanonical);
    assert.strictEqual(intelligenceRegistry.discoveryEngine, intelligenceDiscovery);
    assert.strictEqual(intelligenceRegistry.genericMappingEngine, genericMapping);
    assert.strictEqual(intelligenceRegistry.genericVocabularyEngine, genericVocabulary);
    assert.strictEqual(intelligenceRegistry.genericTissEngine, tiss);
    assert.strictEqual(intelligenceRegistry.genericTissIntegrationEngine, tissIntegration);
    assert.strictEqual(intelligenceRegistry.workflowEngine, workflow);
    assert.strictEqual(intelligenceRegistry.masterOrchestrationEngine, masterOrchestration);
  });

  it("contem apenas oito propriedades readonly, construtor e getCapabilities", () => {
    const keys = Object.keys(intelligenceRegistry);
  ...
  it("getCapabilities ativa Discovery, Canonical e Registry", () => {
    const caps = intelligenceRegistry.getCapabilities();
  ...
  it("nao possui logica funcional: getCapabilities eh o unico metodo", () => {
    const proto = Object.getPrototypeOf(intelligenceRegistry);
  ...
  it("consome apenas Canonical, Discovery e os Gateways oficiais", () => {
    assert.ok(intelligenceRegistry.canonicalEngine);
    assert.ok(intelligenceRegistry.discoveryEngine);
    assert.ok(intelligenceRegistry.genericMappingEngine);
    assert.ok(intelligenceRegistry.genericVocabularyEngine);
