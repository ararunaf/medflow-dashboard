...
describe("J-01 EnterpriseCommandEngine", () => {
  ...
  const engine = new EnterpriseCommandEngine(business, integration, tiss, tissIntegration, workflow);

  it("constrói engine com as fachadas canônicas E, F, G, H e I", () => {
  ...
  it("getCapabilities retorna enterpriseCommandImplemented = true", () => {
  ...
  it("todas as demais capabilities do Bloco J permanecem false", () => {
    const caps = engine.getCapabilities();
    assert.strictEqual(caps.enterpriseOrchestrationImplemented, false);
    assert.strictEqual(caps.enterpriseSagaImplemented, false);
  ...
  it("não duplica lógica de negócio, XML, SOAP, TISS, integração ou workflow", () => {
    const source = engine.toString();
    assert.doesNotMatch(source, /XML/i);
    assert.doesNotMatch(source, /SOAP/i);
    assert.doesNotMatch(source, /TissKnowledge/i);
    assert.doesNotMatch(source, /Communication/i);
    assert.doesNotMatch(source, /Workflow/i);
  });
});
