...
const COMMON_PACIENTE: TemplateFieldDef[] = [
  ...
  },
  {
    ...
    group: "paciente",
    labelPatterns: [/n[uú]mero\s*(da\s*)?carteirinha/i, /carteirinha\s*:/i, /cart[eã]o\s*nacional\s*de\s*sa[uú]de/i],
...
const COMMON_AUTORIZACOES: TemplateFieldDef[] = [
  {
    ...
    group: "autorizacoes",
    labelPatterns: [/senha/i, /senha\s*(de\s*)?autoriza[cç][aã]o/i, /n[uú]mero\s*(da\s*)?guia\s*operadora/i],
...
export const TEMPLATE_SADT_V1: TissTemplate = {
  ...
  guideType: "guia_sadt",
  version: "1.0",
  headerPatterns: [/sp\s*[\/\\]\s*sadt/i, /solicita[cç][aã]o\s*(de\s*)?procedimento/i, /guia\s*sp/i, /sadt/i],
  fields: [
    ...COMMON_OPERADORA,
    ...COMMON_PRESTADOR,
    ...COMMON_PACIENTE,
