/**
 * PoC Application — depende apenas de XMLSchemaRuntimePort (TISS-07).
 *
 * Não é usado por rotas, Server Functions, UI, Upload, Scanner,
 * Contratos, Rule Engine, Workflow ou IA.
 */
import type { XMLSchemaRuntimePort } from "../ports/xml-schema-runtime-port";
import type {
  XMLSchemaRuntimeHealth,
  XMLSchemaRuntimeInfo,
  XMLSchemaRuntimePortCapabilities,
} from "../ports/types";

export type XMLSchemaRuntimeHealthSummary = {
  health: XMLSchemaRuntimeHealth;
  capabilities: XMLSchemaRuntimePortCapabilities;
  info: XMLSchemaRuntimeInfo;
  architectureLayer: "application";
};

/**
 * Query de aplicação: resume saúde/capacidades/info via Port.
 * Zero conhecimento de XSD oficial / validação / XML TISS/ANS / operadoras.
 */
export async function getXMLSchemaRuntimeHealthSummary(
  port: XMLSchemaRuntimePort,
): Promise<XMLSchemaRuntimeHealthSummary> {
  const health = await port.health();
  const capabilities = port.capabilities();
  const info = port.providerInfo();
  return {
    health,
    capabilities,
    info,
    architectureLayer: "application",
  };
}
