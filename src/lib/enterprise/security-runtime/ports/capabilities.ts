/**
 * SecurityRuntimeEngineCapabilities — capacidades declarativas (S1-02).
 *
 * Apenas declaração estrutural. Sem segurança real. Sem criptografia.
...
import type { SecurityCapabilities } from "./canonical";
...
export function emptySecurityRuntimeEngineCapabilities(): SecurityRuntimeEngineCapabilities {
  return {};
}

export function defineSecurityRuntimeEngineCapabilities(
  capabilities: SecurityRuntimeEngineCapabilities = {},
): SecurityRuntimeEngineCapabilities {
  return { ...capabilities };
}

export const DEFAULT_SECURITY_RUNTIME_ENGINE_CAPABILITIES: SecurityRuntimeEngineCapabilities = {
...
export const DEFAULT_MOCK_SECURITY_RUNTIME_ENGINE_CAPABILITIES: SecurityRuntimeEngineCapabilities =
  {
    ...DEFAULT_SECURITY_RUNTIME_ENGINE_CAPABILITIES,
  };

export function toCanonicalSecurityCapabilities(
  capabilities: SecurityRuntimeEngineCapabilities = DEFAULT_SECURITY_RUNTIME_ENGINE_CAPABILITIES,
  ...
  return {
    ...
    supportsCanonicalSecurity: capabilities.supportsCanonicalSecurity === true,
    runtimeReady: true,
    securityEngineImplemented: false,
    businessRulesImplemented: false,
    tissSecurityImplemented: false,
