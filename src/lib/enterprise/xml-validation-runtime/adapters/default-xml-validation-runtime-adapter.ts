...
export class DefaultXMLValidationRuntimeAdapter implements XMLValidationRuntimePort {
  ...
  async repairXML(input: RepairXMLInput): Promise<RepairXMLResult> {
    return this.runOperation("repairXML", input, async () => {
      const repair = this.xmlRepairEngine.repair(input.document, { rules: input.rules });
      return {
        ok: repair.ok,
        repair,
  ...
  async generateXMLValidationReport(
    ...
    return this.runOperation("generateXMLValidationReport", input, async () => {
      const report = this.xmlValidationReportEngine.generate({
